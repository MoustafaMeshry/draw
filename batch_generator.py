...
class BatchGenerator:
    ...
    def __init__(self, batch_size, read_dir=None):
        ...
        if(self.mnist):
            ...
            self.train_data = mnist.input_data.read_data_sets(
    ...
    def next(self, direction=const.Direction.UP, debug=False):
        if(self.mnist):
            ...
            xtrain, _ = self.train_data.next_batch(self.batch_size)
        else:
            img_dims = const.A
            xtrain = np.zeros((self.batch_size, img_dims * img_dims * 3),
                              dtype=np.float32)
            ytrain = np.zeros((self.batch_size, img_dims * img_dims * 3),
                              dtype=np.float32)
            if(debug):
                np.random.seed(10)

            ran_imgs = np.random.randint(len(self.img_list),
            ...
            for i in range(self.batch_size):
                ...
                row, col = img_dims + pts[i, 0:2] * np.subtract(
                            current_img.shape[0:2], (3*img_dims, 3*img_dims))
                # row, col = pts[i, 0:2] * np.subtract(
                #               current_img.shape[0:2], (img_dims, img_dims))
                ...
                if direction == const.Direction.UP.value:
                    img_y = current_img[row-img_dims:row,
                                        col:col+img_dims, :]
                elif direction == const.Direction.DOWN.value:
                    img_y = current_img[row + img_dims:row + 2 * img_dims,
                                        col:col + img_dims, :]
                elif direction == const.Direction.RIGHT.value:
                    img_y = current_img[row:row+img_dims,
                                        col+img_dims:col+2*img_dims, :]
