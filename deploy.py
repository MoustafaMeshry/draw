...
if __name__ == '__main__':
    ...
    for t in range(T):
        ...
        for i in range(xtrain.shape[0]):
            # print(A,B,A*B*3)
            img[0:B, i * A:(i + 1) * A, :] = np.reshape(
                                ytrain[i, :], (B, A, 3)) * intensity_multiplier
            img[B+4:2*B+4, i * A:(i + 1) * A, :] = np.reshape(
                                xtrain[i, :], (B, A, 3)) * intensity_multiplier

            im = (np.reshape(y_recons[t, i, :], (B, A, 3)) *
                  intensity_multiplier).astype(np.uint8)
            if(is_result_sharpen):
                kernel = np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]])
                im = cv2.filter2D(im, -1, kernel)

            img[2 * B + 10:3 * B + 10, i * A:(i + 1) * A, :] = im
            img[3*B+10:4*B+10, i * A:(i + 1) * A, :] = (
                        np.reshape(xtrain[i, :], (B, A, 3)) *
                        intensity_multiplier).astype(np.uint8)
