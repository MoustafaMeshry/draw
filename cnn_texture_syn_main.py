...
if __name__ == '__main__':
    with tf.Session() as sess:
        ...
        for i in range(1):
            ...
            # print('DBG: inp_img shape = ', inp_img.shape, type(inp_img),
            #       type(inp_img[0][0][0]))
            print('DBG: saved input image to %s!' % inp_save_path)
            OUT_PATH = DIR_PATH + '/../output/out_%d_%.0f.jpg' % (
                                                                i, time.time())

            # Initialize variable image that'll become our final output as
            # random noise
            # TODO: try initializing noise to zeros, to see if this would
            # still work similar to draw
            ...
            with tf.name_scope('loss'):
                ...
                if TEXTURE_WEIGHT is 0:
                    ...
                    unweighted_texture_loss = get_texture_loss(x_model,
                                                               texture_model)
                ...
                if NORM_WEIGHT is 0:
                    ...
                    norm_loss = NORM_WEIGHT * (get_l2_norm_loss(noise) **
                                               NORM_TERM)
                ...
                if TV_WEIGHT is 0:
                    ...
                    tv_loss = TV_WEIGHT * get_total_variation(noise,
                                                              image_shape)
            ...
            with tf.name_scope('update_image'):
                ...
                clipped_grads = [(tf.clip_by_value(grad, -1., 1.), var) for
                                 grad, var in grads]
            ...
            logging.info("Training complete. The session took %.2f seconds to"
                         "complete." % elapsed)
            logging.info("Rendering final image and closing TF session..")

            # Render the image after making sure the repo's dedicated output
            # dir exists
