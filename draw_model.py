...
class DrawModel:
    ...
    def __init__(self,batch_size,FLAGS):
        ...
        self.x = tf.placeholder(tf.float32, shape=(batch_size, const.img_size))
        self.y = tf.placeholder(tf.float32, shape=(batch_size, const.img_size))
        self._e = tf.random_normal((batch_size, const.z_size), mean=0, stddev=1)  # Qsampler noise
        ...
        # initial states
        h_dec_prev = tf.zeros((batch_size, const.dec_size))
        enc_state = self._lstm_enc.zero_state(batch_size, tf.float32)
        dec_state = self._lstm_dec.zero_state(batch_size, tf.float32)

        # ==DRAW MODEL== #

        # construct the unrolled computational graph
        for t in range(const.T):
            c_prev = tf.zeros((batch_size, const.img_size)) if t == 0 else self.cs[t - 1]
