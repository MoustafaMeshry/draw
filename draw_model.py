...
class DrawModel:
    ...
    def __init__(self, read_attn, write_attn):
        ...
        self.x = tf.placeholder(tf.float32,
                                shape=(const.batch_size, const.img_size))
        self.y = tf.placeholder(tf.float32,
                                shape=(const.batch_size, const.img_size))
        self._e = tf.random_normal((const.batch_size, const.z_size), mean=0,
                                   stddev=1)  # Qsampler noise
        self._lstm_enc = tf.contrib.rnn.LSTMCell(
                            const.enc_size, state_is_tuple=True)  # encoder Op
        self._lstm_dec = tf.contrib.rnn.LSTMCell(
                            const.dec_size, state_is_tuple=True)  # decoder Op
